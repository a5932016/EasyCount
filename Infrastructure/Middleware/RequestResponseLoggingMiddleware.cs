...
namespace Infrastructure.Middleware
{
    ...
    public class RequestResponseLoggingMiddleware
    {
        ...
        {
            #region 這裡可以加入正則驗證context.Path。過濾不需要紀錄日誌的API
            ...
            {
                // 啟用倒帶功能，就可以讓 Request.Body 可以再試讀取
        ...
        private async Task CatchNext(HttpContext context)
        {
            ...
            {
                _log.LogError(ex, "系統錯誤日誌，管道捕獲");
                context.Response.StatusCode = 200;
                context.Response.ContentType = "application/json; charset=utf-8";
                var result = new { code = 500, message = ex.Message ?? "系統錯誤，請稍後再試" };
            ...
            catch (Exception ex)
            {
                ...
                context.Response.StatusCode = 200;
                context.Response.ContentType = "application/json; charset=utf-8";
                var result = new { code = 500, message = "系統錯誤，請稍後再試" };
                await context.Response.WriteAsync(JsonHelper.Instance.Serialize(result));
            }
        }
